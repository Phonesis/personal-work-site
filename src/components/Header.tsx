...
export default function Header() {
  ...
  // Measure expanded header height for the placeholder
  useEffect(() => {
    if (menuRef.current) {
    ...
    const handleResize = () => {
      if (!isMinimizedRef.current && menuRef.current) {
        setHeaderHeight(menuRef.current.offsetHeight);
      }
    };
  ...
  useEffect(() => {
    ...
    const syncMinimized = () => {
      ...
      if (prev && y <= RESTORE_AT) {
        isMinimizedRef.current = false;
        setIsMinimized(false);
        setMobileMenuOpen(false);
      } else if (!prev && y > MINIMIZE_AT) {
  ...
  const isFixed = headerHeight !== null;
  ...
  return (
    <>
      ...
      <header
        ...
        } top-0 left-0 right-0 z-50 w-full bg-gray-900 text-white shadow-lg transition-all duration-300 ease-in-out ${
